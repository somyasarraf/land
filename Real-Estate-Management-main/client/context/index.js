...
const connectingWithSmartContract = async () => {
  try {
    ...
    const real = new ethers.Contract(address, REAL_ESTATE_ABI, PROVIDER);
...
const buyPropertyFunction = async (buying) => {
  ...
  try {
    ...
    // Convert amount to Wei using parseUnits to ensure consistency
    ...
    const transaction = await contract.buyProperty(productID, address, {
      value: amountInWei, // Use amount directly
    });
  ...
  //   const contract = await connectingWithSmartContract();
  //   console.log("check", contract.address)
  ...
  const getPropertiesData = async () => {
    try {
        ...
        const properties = await contract?.getAllProperties();
        console.log("Properties retrieved successfully:", properties);
        ...
        // You can handle the error here, such as returning a default value or rethrowing
        throw error;
    }
};
